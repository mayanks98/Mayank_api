...
MONGODB_URI = "mongodb://test:test123@ds025409.mlab.com:25409/student_db"
...
def student_list():
	if request.method == 'GET':
		...
		try:
			...
			if student_record.count_documents({'roll_no': roll_no}):
				return json.dumps({'error': 'This record already exists!'})

			# create new record in DB
			student_record.insert_one(data)
			return json.dumps({'result': 'Successfully added the data'})
...
def student_detail(roll_no):
	...
	if request.method == 'GET':
		...
		return json.dumps({'result': 'Successfully update the record'})
	else:
